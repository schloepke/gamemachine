...
public static ClientManagerRegister parseFrom(byte[] bytes) {
	ClientManagerRegister message = new ClientManagerRegister();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(ClientManagerRegister.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<ClientManagerRegister> schema = RuntimeSchema.getSchema(ClientManagerRegister.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(ClientManagerRegister.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(ClientManagerRegister.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
