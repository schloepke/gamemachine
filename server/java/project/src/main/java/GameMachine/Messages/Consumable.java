...
public static Consumable parseFrom(byte[] bytes) {
	Consumable message = new Consumable();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(Consumable.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<Consumable> schema = RuntimeSchema.getSchema(Consumable.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(Consumable.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(Consumable.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
