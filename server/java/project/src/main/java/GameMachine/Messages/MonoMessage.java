...
public static MonoMessage parseFrom(byte[] bytes) {
	MonoMessage message = new MonoMessage();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(MonoMessage.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<MonoMessage> schema = RuntimeSchema.getSchema(MonoMessage.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(MonoMessage.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(MonoMessage.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
