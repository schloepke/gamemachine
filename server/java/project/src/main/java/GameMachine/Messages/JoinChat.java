...
public static JoinChat parseFrom(byte[] bytes) {
	JoinChat message = new JoinChat();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(JoinChat.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<JoinChat> schema = RuntimeSchema.getSchema(JoinChat.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(JoinChat.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(JoinChat.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
