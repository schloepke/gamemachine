...
public static JsonEntity parseFrom(byte[] bytes) {
	JsonEntity message = new JsonEntity();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(JsonEntity.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<JsonEntity> schema = RuntimeSchema.getSchema(JsonEntity.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(JsonEntity.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(JsonEntity.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
