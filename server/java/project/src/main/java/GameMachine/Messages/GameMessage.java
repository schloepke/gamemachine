...
public static GameMessage parseFrom(byte[] bytes) {
	GameMessage message = new GameMessage();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(GameMessage.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<GameMessage> schema = RuntimeSchema.getSchema(GameMessage.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(GameMessage.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(GameMessage.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
