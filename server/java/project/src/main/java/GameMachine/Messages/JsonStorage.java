...
public static JsonStorage parseFrom(byte[] bytes) {
	JsonStorage message = new JsonStorage();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(JsonStorage.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<JsonStorage> schema = RuntimeSchema.getSchema(JsonStorage.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(JsonStorage.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(JsonStorage.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
