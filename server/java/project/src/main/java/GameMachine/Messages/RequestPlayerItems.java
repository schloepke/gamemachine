...
public static RequestPlayerItems parseFrom(byte[] bytes) {
	RequestPlayerItems message = new RequestPlayerItems();
	ProtobufIOUtil.mergeFrom(bytes, message, RuntimeSchema.getSchema(RequestPlayerItems.class));
	return message;
}
...
public byte[] toPrefixedByteArray() {
  ...
  Schema<RequestPlayerItems> schema = RuntimeSchema.getSchema(RequestPlayerItems.class);
...
public byte[] toProtobuf() {
	...
	try {
		bytes = ProtobufIOUtil.toByteArray(this, RuntimeSchema.getSchema(RequestPlayerItems.class), buffer);
		buffer.clear();
	} catch (Exception e) {
		e.printStackTrace();
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, RuntimeSchema.getSchema(RequestPlayerItems.class));
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed");
	}
