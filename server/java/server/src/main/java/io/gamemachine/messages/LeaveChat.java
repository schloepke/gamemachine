...
private static final Logger logger = LoggerFactory.getLogger(LeaveChat.class);
    ...
    static final String defaultScope = LeaveChat.class.getSimpleName();
    ...
    {
    	System.out.println("START LeaveChat");
    	    	//if(this.chatChannel != null) {
    		System.out.println("chatChannel="+this.chatChannel);
    	//}
    	    	System.out.println("END LeaveChat");
    }
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, LeaveChat.getSchema());
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed "+e.getMessage());
	}
