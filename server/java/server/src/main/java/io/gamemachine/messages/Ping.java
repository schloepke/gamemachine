...
public final class Ping implements Externalizable, Message<Ping>, Schema<Ping>, PersistableMessage{
...
private static final Logger logger = LoggerFactory.getLogger(Ping.class);
	...
	public static PingStore store() {
		return PingStore.getInstance();
	}
	...
	static class CacheRef {
		...
		public Ping result(int timeout) {
			ActorSelection sel = ActorUtil.findLocalDistributed("cacheUpdateHandler", id);
	...
	public static class PingCache {

		private static HashMap<String, Field> cachefields = new HashMap<String, Field>();
		private static Cache<String, Ping> cache = new Cache<String, Ping>(120, 5000);
	    ...
	    public void init(int expiration, int size) {
			cache = new Cache<String, Ping>(expiration, size);
		}
	
		public Cache<String, Ping> getCache() {
			return cache;
		}
		...
		private CacheRef updateField(String id, String field, Object value, int updateType) {
			CacheUpdate cacheUpdate = new CacheUpdate(PingCache.class, id, value, field, updateType);
			return new CacheRef(cacheUpdate,id);
		}
		
		public CacheRef set(Ping message) {
			CacheUpdate cacheUpdate = new CacheUpdate(PingCache.class, message);
			return new CacheRef(cacheUpdate,message.id);
		}
	
		public Ping get(String id, int timeout) {
			Ping message = cache.get(id);
			if (message == null) {
				message = Ping.store().get(id, timeout);
			}
			return message;
		}
			
		public static Ping setFromUpdate(CacheUpdate cacheUpdate) throws IllegalArgumentException, IllegalAccessException  {
			Ping message = null;
			String field = cacheUpdate.getField();
			if (field == null) {
				message = (Ping) cacheUpdate.getMessage();
				if (message == null) {
					throw new RuntimeException("Attempt to store empty message in cache");
				}
				cache.set(message.id, message);
				Ping.store().set(message);
			} else {
				message = Ping.cache().get(cacheUpdate.getId(), 10);
				if (message == null) {
					throw new RuntimeException("Cannot set field on null message");
				}
				if (!cachefields.containsKey(field)) {
	            	try {
						cachefields.put(field, Ping.class.getField(field));
	...
	public static class PingStore {
	
		private PingStore() {
		}
		
		private static class LazyHolder {
			private static final PingStore INSTANCE = new PingStore();
		}
	
		public static PingStore getInstance() {
			return LazyHolder.INSTANCE;
		}
	    ...
	    public void set(String scope, Ping message) {
	    	Ping clone = message.clone();
		...
		public Ping get(String scope, String id, int timeout) {
			String scopedId = scopeId(scope,id);
			ObjectdbGet get = new ObjectdbGet().setEntityId(scopedId).setKlass("Ping");
			ActorSelection sel = ActorUtil.findDistributed("object_store", scopedId);
			Timeout t = new Timeout(Duration.create(timeout, TimeUnit.MILLISECONDS));
			AskableActorSelection askable = new AskableActorSelection(sel);
			Ping message = null;
...
public ByteBuf toByteBuf() {
	...
	try {
		ProtobufIOUtil.writeTo(buffer, this, Ping.getSchema());
	} catch (Exception e) {
		e.printStackTrace();
		throw new RuntimeException("Protobuf encoding failed "+e.getMessage());
	}
