    ...
    public void channelRead0(ChannelHandlerContext ctx, FullHttpRequest req) throws Exception {
        ...
        if (req.getMethod() == POST) {
            ...
            if (req.getUri().startsWith("/api")) {
                ...
                if (req.getUri().startsWith("/api/characters/factions")) {
                    FactionStandings factionStandings = FactionService.instance().getFactionStandings(params.get("characterId"));
                    byte[] resp = factionStandings.toByteArray();
                    Ok(ctx, resp);
                    return;
                }
