...
public class VitalsHandler extends GameMessageActor {
    ...
    public static VitalsProxy fromTrackData(TrackData trackData, String zone) {
        ...
        } else if (trackData.entityType == TrackData.EntityType.Boat) {
            return get(trackData.id, Vitals.VitalsType.Boat, zone);
    ...
    public void onGameMessage(GameMessage gameMessage) {
        if (gameMessage.vitalsRequest != null) {
            ...
            if (request.vitalsType == Vitals.VitalsType.Character) {
                Player player = PlayerService.getInstance().find(request.entityId);
                if (player == null) {
                    logger.warn("Player not found: " + request.entityId);
                    return;
                }
