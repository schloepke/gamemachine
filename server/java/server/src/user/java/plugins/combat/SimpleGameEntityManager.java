...
import plugins.core.FactionService;
...
public class SimpleGameEntityManager implements GameEntityManager {
    ...
    @Override
    public void combatDamage(VitalsProxy originProxy, VitalsProxy targetProxy, int damage, CombatLog.Type type) {
        ...
        if (targetProxy.get("health") <= 0) {
            Character them = CharacterService.instance().find(targetProxy.getCharacterId());
            Character me = CharacterService.instance().find(originProxy.getCharacterId());

            FactionStanding standing = FactionService.instance().getStanding(originProxy.getCharacterId(), them.faction);
            standing.standing -= 1f;
            FactionService.instance().saveStanding(standing);

            GameMessage gameMessage = new GameMessage();
            gameMessage.factionStandings = new FactionStandings();
            gameMessage.factionStandings.addStandings(standing);
            PlayerMessage.tell(gameMessage, me.playerId);
        }
    }
