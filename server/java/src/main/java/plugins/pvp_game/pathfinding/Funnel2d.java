...
public class Funnel2d {
	...
	public static List<Vector3> smoothPath(Node startNode, Node endNode, Array<Node> paths) {
		startNode.position = new Vector3(startNode.x, startNode.y, 0d);
		endNode.position = new Vector3(endNode.x, endNode.y, 0d);
		for (Node node : paths) {
			node.position = new Vector3(node.x, node.y, 0f);
		}
