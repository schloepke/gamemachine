...
public class GameMachineLoader {
	...
	public static void StartMessageGateway() {
		actorSystem.actorOf(new RoundRobinPool(200).props(Props.create(MessageGateway.class)), 
			    MessageGateway.name);
	}
	
	public static void StartEntityTracking() {
		actorSystem.actorOf(new RoundRobinPool(200).props(Props.create(EntityTracking.class)), 
			    EntityTracking.name);
	}
