    ...
    public class Actor : MonoBehaviour
    {
        ...
        public virtual void UserUpdate(User user)
        {
            ...
            if (Colliding)
            {
                ...
                if (userVelocity.z > 0f)
                    State = walk ? "WalkForward" : "RunForward";
                else if (userVelocity.z < 0f)
                    State = walk ? "WalkBack" : "RunBack";
                else if (userVelocity.x > 0f)
                    State = "StrafeRight";
                else if (userVelocity.x < 0f)
                    State = "StafeLeft";
                else
                {
                    ...
                    if (Mathf.Abs(userAngular.y) <= 1f)
                        State = "Idle";
                    else 	  if (userAngular.y > 1f)
                        State = "TurnRight";
                    else 	  if (userAngular.y < 1f)
                        State = "TurnLeft";
                }
            ...
            } else if (type != Type.Ground)
            {
                ...
                if (State != "Fly")
                {
                    ...
                    if (userVelocity.y != 0 && airTime == canFlyAtTime)
                        State = "Fly";
                } else
                {
                    ...
                    if (type == Type.Hover)
                        velocity.y += userVelocity.y * MaxScaledSpeed * 2f;
                    else
                        velocity.y += userVelocity.y * MaxScaledSpeed * 0.5f;
                }
            }
        }
        ...
        {	
            ...
            if (!player && target != null)
            {
                ...
                State = "Idle";
                if (watchTarget)
                    User.LookAt2D(transform, target.transform.position);
            }
