    ...
    public class User : MonoBehaviour
    {
        ...
        public virtual Actor Player {
            ...
            set {
                ...
                if (onPlayerAttached != null)
                    onPlayerAttached (value, player);
        ...
        protected virtual void Start ()
        {
            ...
            try {
                ...
                Debug.Log ("InputManager does not support WALK key, using L/R SHIFT" + ": " + e.Message);
                walkInput = false;
            }
            try {
                Input.GetAxis ("Strafe");
            } catch (Exception e) {
                Debug.Log ("InputManager does not support STAFE keys, using Q/E" + ": " + e.Message);
                sideInput = false;
            }
            try {
                Input.GetAxis ("AutoMove");
            } catch (Exception e) {
                Debug.Log ("InputManager does not support AUTOMOVE key, using PAGEUP" + ": " + e.Message);
                autoInput = false;
            }
        }
        ...
        public bool Attached (Actor player)
        {
        ...
        public static void PrintPositions (Transform transform)
        {
            ...
            if (!Application.isPlaying && Input.GetKeyDown (KeyCode.C)) {
                foreach (Vector3 m in positions)
                    if (m == transform.position)
                        return;
